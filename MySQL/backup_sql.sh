...
# === CONFIG ===
BCKDIR='/var/backup/mysql'    # backup destination
MYCNF='/etc/mysql/debian.cnf' # mysql account for dumps
...
for BDD in `mysql --defaults-file=${MYCNF} --skip-column-names -B -e "SHOW databases;" | egrep -v "^information_schema$|^performance_schema$"`; do
    ...
    mysqldump --defaults-file=${MYCNF} --set-gtid-purged=OFF --routines --no-create-info --no-data --no-create-db --skip-opt ${BDD} > ${DST}/${BDD}-routines.sql
    ...
    for TABLE in `mysql --defaults-file=${MYCNF} --skip-column-names -B ${BDD} -e "SHOW TABLES;" | grep -v slow_log | grep -v general_log`; do
        ...
        mysqldump --defaults-file=${MYCNF} --set-gtid-purged=OFF --single-transaction ${BDD} ${TABLE} > "${DST}"/"${BDD}"/"${TABLE}".sql
